...
**Custom meals from the browser.** The Fuel tab lets the user log a new meal
(name, portion, kcal, protein). It's pushed to the inbox like any meal, but the
payload carries `save: true` and a structured `meal` object. On drain, when you
see `save: true`: add that meal to the journal/meals.md **Library** (and to the
**Quick meals** shortlist if it's a genuine repeat), so the next snapshot serves
it as a one-tap button. The browser caches nothing — the inbox is the only
write path. Treat the user's entered numbers as their own estimate; don't
override them, but flag anything implausible.
